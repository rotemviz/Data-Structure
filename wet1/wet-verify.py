...
files_to_include = ["wet1util.h", "main24b1.cpp"]
...
def main_verify():
	...
	if not os.path.exists(path_to_zip):
		print("input zip file does not exist: "+path_to_zip)
		exit(1)

	if not os.path.isfile(path_to_zip):
		print("input path is not a file: "+path_to_zip)
		exit(1)
	
	zip_extract(path_to_zip, zipdir)
	nerr = 0;
	...
	if nerr == 0:
		if len(sys.argv) > 2:
			if "--hash" in sys.argv:
				print("The submission file has hash: " + str(hash(open(path_to_zip, "r").read())))
		print("It is OK to submit :)")
	else:
